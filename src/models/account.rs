...
impl Account {
    ...
    pub async fn update(&self, client: &Client) -> Result<()> {
        client.query("UPDATE accounts SET name = $1, amount = $2, debt = $3 WHERE id = $5 AND user_id = $6", &[&self.name, &self.amount, &self.debt, &self.id, &self.user_id]).await?;
        Ok(())
    }
    ...
    pub async fn get_all(client: &Client, user_id: i32) -> Result<Vec<Self>> {
        let rows = client
            .query(
                "SELECT accounts.* FROM goals INNER
